...
def load_configuration():
    ...
    missing = [key for key, value in config.items() if key != "cookies_filepath" and value is None]
    if missing:
        logging.error(f"Missing environment variables: {', '.join(missing)}")
        raise ValueError("Configuration incomplete. Please set the missing environment variables.")
    return config
...
def get_already_downloaded(music_folderpath):
    ...
    downloaded = [file.name for file in Path(music_folderpath).glob("*.mp3") if file.is_file()]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download liked songs from YouTube Music.")
    parser.add_argument(
        "--env-file",
        default=".env",
    ...
    with logging_redirect_tqdm():
        ...
        if failed_urls and config.get("cookies_filepath"):
            ...
            failed_urls.clear()
            logging.info(f"Retrying {len(retry_list)} failed downloads with cookies enabled...")
            ytdl_opts_retry = create_youtube_dl_options(config, use_cookies=True)
            download_songs(retry_list, ytdl_opts_retry)
            if failed_urls:
                logging.error(f"After retry, {len(failed_urls)} downloads still failed: {failed_urls}")
            else:
                logging.info("All failed downloads succeeded on retry.")
        elif failed_urls:
            logging.error(f"Downloads failed for {len(failed_urls)} songs: {failed_urls}")
